        ...
        <section className="mt-7 bg-card border border-line rounded-card px-5">
          <table className="w-full border-collapse"><tbody>{projects.map((project) => (
            <tr key={project.applicationPublicId} className="border-b border-line-2 last:border-b-0">
              <td className="py-3 pr-3"><Link href={`/workspace/${project.applicationPublicId}`} className="font-medium text-ink hover:text-brand">{project.challengeTitle}</Link><p className="text-meta text-ink-3 mt-0.5">{projectStatusLabel(project.projectStatus)}</p></td>
