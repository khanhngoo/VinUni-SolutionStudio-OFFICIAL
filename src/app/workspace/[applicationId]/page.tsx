...
import { milestoneReviewDecisionLabel, milestoneStatusLabel, projectStatusLabel, resourceSensitivityLabel } from "@/lib/labels";
...
export default async function WorkspacePage({ params, searchParams }: { params: Promise<{ applicationId: string }>; searchParams: Promise<RawSearchParams> }) {
  ...
  return <div className="max-w-[980px] mx-auto px-6 sm:px-7 py-7 pb-16">
    ...
    <div className="flex flex-wrap gap-1.5 mt-3.5 mb-2.5"><Chip variant="ok">{projectStatusLabel(detail.projectStatus)}</Chip><Chip>{detail.challenge.ownerOrganizationName}</Chip></div>
    ...
    {tab === "overview" ? <>
      <Section title="Where things stand"><dl className="grid grid-cols-2 sm:grid-cols-4 gap-2.5"><Stat label="Progress" value={`${detail.progress.completed} / ${detail.progress.total}`} /><Stat label="Next due" value={detail.nextDeadline ? formatDate(detail.nextDeadline) : "—"} /><Stat label="Workload" value={detail.challenge.weeklyHours ? `${detail.challenge.weeklyHours} hrs / wk` : "—"} /><Stat label="Status" value={projectStatusLabel(detail.projectStatus)} /></dl></Section>
